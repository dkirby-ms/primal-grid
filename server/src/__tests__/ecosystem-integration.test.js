...
import { CREATURE_TYPES, CREATURE_AI, RESOURCE_REGEN, } from "@primal-grid/shared";
// ── Helpers ─────────────────────────────────────────────────────────
function createRoomWithEcosystem(seed) {
...
function findResourceTile(room) {
    for (let i = 0; i < room.state.tiles.length; i++) {
        const tile = room.state.tiles.at(i);
...
function findBarrenWalkableTile(room) {
    for (let i = 0; i < room.state.tiles.length; i++) {
        const tile = room.state.tiles.at(i);
...
function countByType(room) {
    ...
    room.state.creatures.forEach((c) => {
        if (c.creatureType === "herbivore")
            herbivores++;
        else if (c.creatureType === "carnivore")
            carnivores++;
    });
    return { herbivores, carnivores };
}
/** Total resource amount across all tiles. */
function totalResources(room) {
    let sum = 0;
    for (let i = 0; i < room.state.tiles.length; i++) {
        const tile = room.state.tiles.at(i);
        if (tile.resourceAmount > 0)
            sum += tile.resourceAmount;
    }
    return sum;
}
/** Manhattan distance. */
...
function findAdjacentWalkablePair(room) {
...
describe("Phase 2.6 — Carnivore Hunting Reduces Herbivore Population", () => {
    ...
    it("carnivore kills herbivore after sufficient attacks", () => {
        ...
        for (let i = 0; i < 30; i++) {
            room.state.tick += CREATURE_AI.TICK_INTERVAL;
            room.tickCreatureAI();
            if (!room.state.creatures.has("doomed"))
                break;
        }
...
//# sourceMappingURL=ecosystem-integration.test.js.map