...
import { CREATURE_TYPES, CREATURE_RESPAWN, CREATURE_SPAWN, DEFAULT_MAP_SIZE, } from "@primal-grid/shared";
// ── Helpers ─────────────────────────────────────────────────────────
...
function claimAllTiles(room, ownerID) {
...
function claimAllWalkableExceptOne(room, ownerID) {
    const w = room.state.mapWidth;
    const h = room.state.mapHeight;
    let spared = null;
    for (let y = 0; y < h; y++) {
...
describe("Territory Exclusion — Creature Spawning", () => {
    ...
    it("tickCreatureRespawn: new creatures avoid owned tiles", () => {
        ...
        room.state.creatures.forEach((c) => existingIds.add(c.id));
        // Trigger respawn
    ...
    it("respawned creatures land on walkable unowned tiles", () => {
        const room = createRoomWithCreatures(42);
        const minPop = CREATURE_TYPES["carnivore"].minPopulation;
        // Claim tiles
...
//# sourceMappingURL=creatureSpawnTerritory.test.js.map