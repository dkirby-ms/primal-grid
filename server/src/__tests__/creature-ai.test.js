...
import { TileType, CREATURE_TYPES, CREATURE_AI, DEFAULT_MAP_SIZE, } from "@primal-grid/shared";
// ── Helpers ─────────────────────────────────────────────────────────
...
function findTilesAtDistance(room, dist, tileType) {
    const w = room.state.mapWidth;
    for (let y1 = 0; y1 < w; y1++) {
        for (let x1 = 0; x1 < w; x1++) {
            if (!room.state.isWalkable(x1, y1))
                continue;
            const t1 = room.state.getTile(x1, y1);
            if (tileType !== undefined && t1?.type !== tileType)
                continue;
            // Try horizontal
            const x2 = x1 + dist;
...
describe("Phase 2.5 — Creature AI: Herbivore Transitions", () => {
    ...
    it("hungry herbivore near food transitions to eat", () => {
        ...
        if (!foodTile)
            return; // skip if no food on this seed
...
describe("Phase 2.5 — Creature AI: Hunger Depletion", () => {
    ...
    it("creature hunger does not go below 0", () => {
        ...
        for (let i = 0; i < 10; i++) {
            room.state.tick += CREATURE_AI.TICK_INTERVAL;
            room.tickCreatureAI();
            if (!room.state.creatures.has("hunger-floor"))
                break;
        }
...
//# sourceMappingURL=creature-ai.test.js.map