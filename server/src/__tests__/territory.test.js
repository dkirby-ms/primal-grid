...
function findClaimableAdjacentTile(room, playerId) {
...
function findNonAdjacentUnownedTile(room, playerId) {
    for (let i = 0; i < room.state.tiles.length; i++) {
        const tile = room.state.tiles.at(i);
        if (tile.ownerID === "" &&
            tile.type !== TileType.Water &&
...
function _findUnwalkableTile(room) {
    for (let i = 0; i < room.state.tiles.length; i++) {
        const tile = room.state.tiles.at(i);
...
describe("Territory System", () => {
    ...
    describe("HQ zone — no water or rock (all 25 tiles claimed)", () => {
         ...
         */
        function buildState(tileTypeFn) {
