...
import { CREATURE_TYPES, CREATURE_AI, } from "@primal-grid/shared";
// ── Helpers ─────────────────────────────────────────────────────────
 ...
 */
function findSpacedWalkableTiles(room, count, minSeparation = 10) {
    const tiles = [];
    ...
    for (let y = 1; y < w - 1 && tiles.length < count; y++) {
        for (let x = 1; x < w - 1 && tiles.length < count; x++) {
            ...
            const hasWalkableNeighbor = room.state.isWalkable(x + 1, y) ||
...
function snapshotPositions(room) {
    const snap = new Map();
...
function countMoved(room, before) {
    let moved = 0;
    room.state.creatures.forEach((c) => {
    ...
    describe("per-creature movement cooldown", () => {
        ...
        it("cooldown is per-creature, not global", () => {
            ...
            for (let t = 0; t < 20; t++) {
                ...
                aiTick(room);
                if (creatureA.x !== prevAx || creatureA.y !== prevAy)
                    moveTicks.a.push(t);
                if (creatureB.x !== prevBx || creatureB.y !== prevBy)
                    moveTicks.b.push(t);
            }
...
//# sourceMappingURL=creature-independent-movement.test.js.map