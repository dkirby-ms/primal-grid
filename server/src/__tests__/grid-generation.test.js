...
function findTileOfType(state, type) {
...
describe("Grid Generation", () => {
    ...
    it("water and rock tiles are not walkable", () => {
        ...
        if (waterTile)
            expect(room.state.isWalkable(waterTile.x, waterTile.y)).toBe(false);
        if (rockTile)
            expect(room.state.isWalkable(rockTile.x, rockTile.y)).toBe(false);
    });
    ...
    it("same seed produces identical maps", () => {
        const room1 = Object.create(GameRoom.prototype);
        room1.state = new GameState();
        room1.generateMap(42);
        const room2 = Object.create(GameRoom.prototype);
...
//# sourceMappingURL=grid-generation.test.js.map