...
function countPlayerBuilders(room, playerId) {
    let count = 0;
...
describe("Builder AI FSM (idle, move_to_site, building)", () => {
    ...
    it("build completes after BUILD_TIME_TICKS — tile owned, shapeHP set, score+XP increase", () => {
        ...
        for (let i = 0; i < PAWN.BUILD_TIME_TICKS + 5; i++)
            tickAI(room);
...
describe("Upkeep system (resource drain, frequency)", () => {
    ...
    it("builder dies from accumulated upkeep damage (removed from creatures)", () => {
        ...
        for (let cycle = 1; cycle <= 20; cycle++) {
            ...
            if (!room.state.creatures.has("b-starve"))
                break;
        }
...
//# sourceMappingURL=pawnBuilder.test.js.map