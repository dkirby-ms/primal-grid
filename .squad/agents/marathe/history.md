...
## 2026-03-09: Auto-Close Workflow for Squash-Merged PRs

**Task:** Fix recurring bug where squash-merged PRs fail to auto-close linked GitHub issues  
**Status:** ✅ Completed  
**File created:** `.github/workflows/auto-close-issues.yml`

### Problem

GitHub's squash-merge rewrites the commit message and can drop `Closes #N` keywords from the PR body. This caused 6+ issues (#19, #30, #31, #42, #74, #77) to stay open after their PRs merged, requiring manual cleanup.

### Solution

Created a GitHub Actions workflow that:
- Triggers on `pull_request` `closed` events when merged
- Parses PR body for `Closes/Fixes/Resolves #N` patterns (case-insensitive, via `grep -ioP`)
- Closes matched issues via `gh issue close --reason completed`
- Adds linking comment on each closed issue
- Idempotent: skips already-closed or not-found issues
- Minimal permissions: `issues: write`, `pull-requests: read`

### Key Patterns

- **PR body is the source of truth** for issue linking — not commit messages
- **Idempotent CI jobs** — always check state before acting (e.g., skip if issue already closed)
- **`grep -ioP` with `\K`** — Perl-compatible regex for clean keyword extraction in shell scripts
- **Repo settings alternative:** GitHub repo setting "Default commit message → PR body" could also help, but it's a UI-only setting not under version control — workflow approach is more reliable

### Files Modified

- `.github/workflows/auto-close-issues.yml` (created)
- `.squad/copilot-instructions.md` (updated PR guidelines)
- `.squad/decisions/inbox/marathe-auto-close-workflow.md` (created)
