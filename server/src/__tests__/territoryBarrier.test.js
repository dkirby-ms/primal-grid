...
import { ResourceType, CREATURE_TYPES, CREATURE_AI, } from "@primal-grid/shared";
// ── Helpers ─────────────────────────────────────────────────────────
...
function claimTile(room, x, y, ownerID) {
...
function findWalkableBlock(room, width, height) {
        ...
        for (let x = 1; x < mapW - width - 1; x++) {
            ...
            for (let dy = -1; dy <= height && allWalkable; dy++) {
                for (let dx = -1; dx <= width && allWalkable; dx++) {
                    if (!room.state.isWalkable(x + dx, y + dy))
                        allWalkable = false;
                }
            }
            if (allWalkable)
...
describe("Territory Barrier — Herbivore blocked", () => {
    it("herbivore wandering does NOT enter a player-owned tile", () => {
        ...
        for (let i = 0; i < 60; i++) {
            ...
            if (!c)
                break; // died — fine, not our concern
...
//# sourceMappingURL=territoryBarrier.test.js.map